...
def test_pipeline(prompt: str):
    """Test the entire pipeline by applying all methods sequentially."""
    print("_count_tokens")
...
def test_individual(prompt: str):
    """Test each method individually without applying others."""
    print("_count_tokens")
    print(optimizer._count_tokens(prompt))
    ...
    print("_remove_request_phrases")  # rem_req
    p = optimizer._remove_request_phrases(prompt)
    print(p)

    print("_remove_filler_words")  # rem_fill
    p = optimizer._remove_filler_words(prompt)
    print(p)

    print("_remove_redundant_pronouns")  # rem_pron
    p = optimizer._remove_redundant_pronouns(prompt)
    print(p)

    print("_remove_stylistic_chars")  # rem_Style
    p = optimizer._remove_stylistic_chars(prompt)
    print(p)

    print("_prepass_replacements")  # prep_rep
    p = optimizer._prepass_replacements(prompt)
    print(p)

    print("_replace_verbose_phrases")  # rem_verb
    p = optimizer._replace_verbose_phrases(prompt)
    print(p)

    print("_reduce_intensifiers")  # intens_redu
    p = optimizer._reduce_intensifiers(prompt)
    print(p)

    print("_extract_main_intent")  # ext_mi
    p = optimizer._extract_main_intent(prompt)
    print(p)

    print("_normalize_commas")
    p = optimizer._normalize_commas(prompt)
    print(p)

    print("_post_cleanup")
    p = optimizer._post_cleanup(prompt)
    print(p)

    print("_clean_whitespace_and_punctuation")
    p = optimizer._clean_whitespace_and_punctuation(prompt)
    print(p)
